...
int main(int argc, char *argv[])
{
	...
	timelimit = (NULL != argv[9]) ? std::stoull(argv[9]) : 0 ;
...
//		nThreads -= 1 ;
	...
	ARE::VarElimOrderComp::ObjectiveToMinimize algCodeSecondary = ARE::VarElimOrderComp::None;
	bool eliminateSingletonDomainVariables = true;
	bool findPracticalVariableOrder = true;
	long int randomSeed = 0; //seed will be randomized
	int ret = ARE::VarElimOrderComp::Compute(uaifile, objCode, algCode, algCodeSecondary, nThreads, nRunsToDo, timelimit, nRandomPick, eRandomPick, PerformSingletonConsistencyChecking, eliminateSingletonDomainVariables, EarlyTerminationOfBasic_W, EarlyTerminationOfBasic_C, findPracticalVariableOrder, randomSeed, BestOrder, cvocontext) ;

	if (0 == ret) {
		INT64 runtime = cvocontext->_tEnd - cvocontext->_tStart ;
		printf("\n\nBEST ORDER w width=%d; TotalVarElimComplexity=%g, TotalNewFunctionStorageAsNumOfElements=%g, nFillEdges=%d, nImprvmnts=%d, nRuns=%d/%d, runtime=%I64dmsec, file=%s", BestOrder._Width, BestOrder._Complexity_Log10, BestOrder._TotalNewFunctionStorageAsNumOfElements_Log10, BestOrder._nFillEdges, cvocontext->_nImprovements, cvocontext->_nRunsCompleted, cvocontext->_nRunsCompleted, runtime, vofile.c_str()) ;
		if (NULL != ARE::fpLOG) {
			fprintf(ARE::fpLOG, "\n\nBEST ORDER w width=%d; TotalVarElimComplexity=%g, TotalNewFunctionStorageAsNumOfElements=%g, nFillEdges=%d, nImprvmnts=%d, nRuns=%d/%d, runtime=%I64dmsec, file=%s", BestOrder._Width, BestOrder._Complexity_Log10, BestOrder._TotalNewFunctionStorageAsNumOfElements_Log10, BestOrder._nFillEdges, cvocontext->_nImprovements, cvocontext->_nRunsCompleted, cvocontext->_nRunsCompleted, runtime, vofile.c_str()) ;
			fprintf(ARE::fpLOG, "\nImprovements list :") ;
		...
		if (NULL != fpVO_w) {
			fprintf(fpVO_w, "#filename=%s, width=%d, TotalVarElimComplexity=%g, TotalNewFunctionStorageAsNumOfElements=%g", uaifile.c_str(), BestOrder._Width, BestOrder._Complexity_Log10, BestOrder._TotalNewFunctionStorageAsNumOfElements_Log10) ;
